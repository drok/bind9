...
REQUIRE(dns_zone_getkeydirectory(zone) != NULL);
	...
	if (zone != NULL) {
		...
		if (view->acache != NULL)
			dns_zone_setacache(zone, view->acache);
//isc_log_write(dns_lctx, DNS_LOGCATEGORY_GENERAL, DNS_LOGMODULE_DNSSEC, ISC_LOG_WARNING, "----------- %s:%u:%s: calling  dns_zonemgr_managezone zone=%p", __FILE__, __LINE__, __FUNCTION__, zone);
//REQUIRE(dns_zone_getkeydirectory(zone) != NULL);
		CHECK(dns_zonemgr_managezone(ns_g_server->zonemgr, zone));
		dns_zone_setstats(zone, ns_g_server->zonestats);
	}
